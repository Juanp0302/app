...
7. **Segundo intento (2026-08-03):** ya con la pantalla de Cobro visible, la página de pago hospedada por Trazo mostró Descripción/Referencia/Código de transacción vacíos y un literal "Se cobrará undefined de recargo adicional" — parece un bug/config faltante del lado de Trazo (sandbox), no de nuestra solicitud. Soporte pidió dos ajustes, ya aplicados: `custom_webhook` por cobro (cada Cobro necesita el suyo, a diferencia de Suscripciones que usa el webhook global) y `channel: 'EMAIL'` en vez de `'LINK'`.
8. **Confirmación manual mientras el webhook no esté registrado:** se agregó `confirmarCobroManual()` en `lib/trazo-cobros-flujo.ts` (reutiliza la misma lógica de activación que usa el webhook, extraída a `activarCuentaPorCobro()`).
   - Primero se armó como un endpoint interno (`POST /api/interno/trazo-cobros`, protegido con `INTERNAL_SECRET`) para que Claude lo invocara por Bash — pero eso exigía compartir el secreto en cada sesión, así que se descartó ese modo de uso.
   - **Decisión final (2026-08-03):** pantalla nueva en el dashboard de superadmin (`app/dashboard/superadmin/SuperadminClient.tsx`, sección "Pagos de Trazo pendientes de confirmación") que lista los cobros en estado `pendiente` (`GET /api/superadmin/trazo-cobros`) con un botón "Confirmar pago y activar" por cada uno (`POST /api/superadmin/trazo-cobros`, mismo patrón de auth `requireSuperadmin` que el resto del panel). El superadmin verifica el pago en el dashboard de Trazo y aprueba desde ahí — sin depender de secretos ni de pedirle a Claude que lo ejecute cada vez.
