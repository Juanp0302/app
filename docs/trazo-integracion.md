...
8. **Confirmación manual mientras el webhook no esté registrado:** se agregó `confirmarCobroManual()` en `lib/trazo-cobros-flujo.ts` (reutiliza la misma lógica de activación que usa el webhook, extraída a `activarCuentaPorCobro()`) y un endpoint interno `POST /api/interno/trazo-cobros` protegido con `INTERNAL_SECRET` (mismo patrón que `/api/superadmin/auto-cuentas-cobro`). **A propósito NO se agregó ninguna pantalla al producto** — el usuario prefiere pedirle a Claude, en el chat, que confirme un pago puntual (dándole el email del cliente o el `external_reference`), y que sea Claude quien llame el endpoint por él. `GET /api/interno/trazo-cobros?email=...` sirve para encontrar la referencia pendiente de un cliente por su correo.
