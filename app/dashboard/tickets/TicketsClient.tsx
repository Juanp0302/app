...
function badge(val: string, arr: { key: string; label: string; color: string }[]) {
  ...
  if (!item) return null
  return <span style={{ fontSize: 11, fontWeight: 700, padding: '3px 10px', borderRadius: 20, background: item.color + '20', color: item.color, border: `1px solid ${item.color}40` }}>{item.label}</span>
}

const FORM_INIT = { tipo: 'financiera', asunto: '', descripcion: '', prioridad: 'normal' }

export default function TicketsClient({ userRole, userId, clienteId }: { userRole: string; userId: string; clienteId: string | null }) {
  ...
  const cargarTickets = useCallback(async () => {
    const url = isAdmin ? '/api/tickets' : `/api/tickets?clienteId=${clienteId}`
  ...
  async function responder() {
    ...
    setEnviando(true)
    await fetch('/api/tickets', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ accion: 'responder', ticketId: activo.id, contenido: texto }) })
    setTexto(''); await cargarDetalle(activo.id); await cargarTickets()
    setEnviando(false)
  }

  async function cambiarEstado(estado: string) {
    await fetch('/api/tickets', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ accion: 'actualizar', ticketId: activo.id, estado }) })
    await cargarDetalle(activo.id); await cargarTickets()
  }

  async function reasignar() {
    if (!adminDest || !activo) return
    await fetch('/api/tickets', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ accion: 'reasignar', ticketId: activo.id, adminId: adminDest, motivo }) })
    setModalReasig(false); setMotivo(''); setAdminDest('')
    await cargarDetalle(activo.id); await cargarTickets()
  }

  const ticketsFiltrados = filtroEstado === 'todos' ? tickets : tickets.filter((t: any) => t.estado === filtroEstado)
  ...
  const btn = (bg: string, color = '#fff'): React.CSSProperties => ({ background: bg, color, border: 'none', borderRadius: 8, padding: '9px 16px', cursor: 'pointer', fontFamily: 'inherit', fontSize: 13, fontWeight: 600 })
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f5f3ee', fontFamily: "'Josefin Sans', sans-serif" }}>
      ...
      <nav style={{ background: C.vino, padding: '0.9rem 2rem', display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
        ...
        <span style={{ fontSize: '0.72rem', fontWeight: 600, letterSpacing: '0.15em', textTransform: 'uppercase', color: C.olivo }}>Tickets</span>
      </nav>

      <div style={{ display: 'grid', gridTemplateColumns: '360px 1fr', height: 'calc(100vh - 52px)' }}>
        ...
        <div style={{ background: '#fff', borderRight: '1px solid #e5e7eb', display: 'flex', flexDirection: 'column' }}>
          <div style={{ padding: '1rem', borderBottom: '1px solid #f0f0f0' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
              <span style={{ fontWeight: 700, color: C.vino, fontSize: 15 }}>Tickets</span>
              {!isAdmin && <button style={btn(C.bordo)} onClick={() => setModalNuevo(true)}>+ Nuevo</button>}
            </div>
            <select value={filtroEstado} onChange={e => setFiltroEstado(e.target.value)} style={{ ...inp, padding: '7px 10px', fontSize: 12 }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto' }}>
            {ticketsFiltrados.length === 0 ? (
              ...
              <div key={t.id} onClick={() => cargarDetalle(t.id)}
                style={{ padding: '0.9rem 1rem', borderBottom: '1px solid #f5f5f5', cursor: 'pointer', background: activo?.id === t.id ? '#fef3c7' : 'transparent' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
                  <span style={{ fontWeight: 600, fontSize: 13, color: C.vino }}>{t.asunto}</span>
                ...
                {isAdmin && t.razon_social && <div style={{ fontSize: 11, color: '#888', marginTop: 4 }}>{t.razon_social}</div>}
                <div style={{ fontSize: 11, color: '#bbb', marginTop: 3 }}>{new Date(t.updated_at).toLocaleDateString('es-CO')}</div>
              </div>
            ))}
          </div>
        </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
            ...
            <div style={{ padding: '1.25rem 1.5rem', background: '#fff', borderBottom: '1px solid #e5e7eb' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 10 }}>
                <div>
                  <h2 style={{ margin: '0 0 6px', color: C.vino, fontSize: 18 }}>{activo.asunto}</h2>
                ...
                {isAdmin && !['resuelto','cerrado'].includes(activo.estado) && (
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    {ESTADOS.filter(e => e.key !== activo.estado && e.key !== 'abierto').map(e => (
                      <button key={e.key} style={btn(e.color + '20', e.color)} onClick={() => cambiarEstado(e.key)}>{e.label}</button>
                    ))}
            ...
            <div style={{ flex: 1, padding: '1.25rem 1.5rem', display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              {historial.length > 0 && (
                <div style={{ borderTop: '1px solid #e5e7eb', paddingTop: 12, marginTop: 8 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: '#999', textTransform: 'uppercase', marginBottom: 8 }}>Historial de reasignaciones</div>
                  {historial.map((h: any) => (
            ...
            {/* Responder */}
            {!['cerrado'].includes(activo.estado) && (
      ...
      {modalNuevo && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: '#fff', borderRadius: 16, padding: '2rem', width: '100%', maxWidth: 500, boxShadow: '0 20px 60px rgba(0,0,0,0.3)', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div>
                ...
                <input value={form.asunto} onChange={e => setForm(f => ({ ...f, asunto: e.target.value }))} style={inp} placeholder="Resumen del problema" />
              </div>
              <div>
