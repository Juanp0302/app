 ...
 * POST /api/recordatorios        — Ejecutar recordatorios manualmente (admin)
 *
 * Este endpoint también puede ser llamado por un cron job externo.
 * Para protegerlo en producción, usa la variable CRON_SECRET.
 */
...
export async function GET(req: NextRequest) {
  ...
  const historial = db.prepare(`
    SELECT
      al.id, al.created_at, al.entidad_id, al.detalle, al.user_email,
      co.estado,
      oc.obligacion, oc.aspecto, oc.periodicidad,
      c.razon_social
    FROM audit_log al
  ...
  // Próximos recordatorios que se deberían enviar
  const proximos = db.prepare(`
    ...
    ORDER BY c.razon_social, oc.aspecto
    LIMIT 200
  `).all() as any[]
...
export async function POST(req: NextRequest) {
  // Permitir llamada desde cron con secret, o desde admin autenticado
  const cronSecret = req.headers.get('x-cron-secret')
  if (cronSecret && cronSecret === process.env.CRON_SECRET) {
    // Llamada automática del cron — proceder
  } else {
    const session = await auth()
    if (!session?.user) return NextResponse.json({ error: 'No autorizado' }, { status: 401 })
    const user = session.user as any
