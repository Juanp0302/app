      ...
      if (!permiso.ok) {
        ...
        if (permiso.razon === 'limite_alcanzado') {
          ...
          return NextResponse.json({
            error: `Alcanzaste el límite de ${p.limite} chat${p.limite !== 1 ? 's' : ''} de tu plan ${p.plan}. Actualiza tu plan para continuar.`,
            codigo: 'LIMITE_CHATS', limite: p.limite, usado: p.usado, plan: p.plan,
          }, { status: 403 })
        }
      }
    }
